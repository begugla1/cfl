...
pub use utils::{collect_user_args, exit_program, get_current_working_dir, get_dir_from_str};

const MAX_FILE_LEN: usize = 255;
const MAX_ABS_PATH_LEN: usize = 4096;
const RESULT_EMOJII: [char; 2] = ['❌', '✅'];

/// Checking if it possible to create file with given name in given directory
/// based on it's length and length of absolute path with this file.
pub fn is_file_len_valid(filename: &str, dir: PathBuf) -> Result<bool, Box<dyn Error>> {
